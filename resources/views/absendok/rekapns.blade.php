<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            <form action="{{ route('rekapabsenns') }}" method="GET">
                ...
                <input type="date" name="sampai" id="sampai" value="{{ request()->get('sampai') ?? date('Y-m-d')}}"
                    class="input input-sm input-bordered">
                <select name="ns" class="select select-sm select-bordered">
                    <option disabled selected>Pilih NS ...</option>
                    @foreach ($ns as $item)
                        <option value="{{$item->kodens}}" {{ (request()->ns == $item->kodens)?'selected':'' }}>{{ $item->namans }}</option>
                    @endforeach
                </select>
                <button type="submit" class="btn btn-sm btn-primary"
                    onclick="this.disabled=true;this.value='Submitting...'; this.form.submit();">view</button>
                <button formaction="{{ route('xrekapabsenns') }}" class="btn btn-sm btn-secondary"
